...
struct base85 {
    ...
    base85(size_t N, const char* out_fn) : N_(N), output_filename_(out_fn) {
    ...
    void matrixInsert(int& r, int& c, array<uint8_t, 4> a) {
        uint8_t cnt = 0;
        for (; r < m_->rows_; r++) {
            for (; r < m_->cols_; c++) {
                m_->operator()(r, c)[0] = a[cnt++];
                m_->operator()(r, c)[1] = a[cnt++];
                m_->operator()(r, c)[2] = a[cnt++];
    ...
    void decode(const char* in_filename) {
        ...
        save_ppm(output_filename_, m_, false);
    }
